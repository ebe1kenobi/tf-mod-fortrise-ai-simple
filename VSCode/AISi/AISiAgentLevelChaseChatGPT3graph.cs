...
//    public string testActionName = "jumpleftm1m0"; // Change ici pour tester une autre action
//    //public float testActionX = 305;
//    //public float testActionY = 222;
//    public float testActionX = 150;
//    public float testActionY = 222;
...
//      }
//      ////////////////////////
//      foreach (String jumpType in new string[] { "jumpleftm", "jumprightp" })
//      {
//        int moveX = jumpType == "jumpleftm" ? -1 : 1;
//        for (int i = 3; i <= 6; i++)
//        {
//          int iteration = i;
//          float duration = 0;
...
//      //  {
//      //    //None
//      //    new MovementAction("None", 0, 0, 0f, 1f,
//      //        (p, ai) => true,
//      //        (p, ai) => new Point(p.X, p.Y)),

//      //    // Marche droite
//      //    new MovementAction("WalkRight", 1, 0, 1f, 1f,
//      //        (p, ai) => ai.IsCellWalkable(p.X + 1, p.Y) && ai.HasGroundBelow(p.X, p.Y),
//      //        (p, ai) => new Point(p.X + 1, p.Y)),

//      //    // Marche gauche
//      //    new MovementAction("WalkLeft", -1, 0, 1f, 1f,
//      //        (p, ai) => ai.IsCellWalkable(p.X - 1, p.Y) && ai.HasGroundBelow(p.X, p.Y),
//      //        (p, ai) => new Point(p.X - 1, p.Y)),

//      //    // Petit saut vertical
//      //    new MovementAction("JumpUp", 0, -3, 1.5f, 1f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X, p.Y - 3)),

//      //    // Petit saut diagonal droite
//      //    new MovementAction("JumpRight", 3, -3, 1.5f, 1f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X + 3, p.Y - 3)),

//      //    // Petit saut diagonal gauche  //duration ok 
//      //    new MovementAction("JumpLeft", -4, -1, 1.5f, 0.6f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X - 3, p.Y - 3)),
//      //    new MovementAction("JumpLeft_m4_m1", -4, -1, 1.5f, 0.6f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X - 3, p.Y - 3)),
//      //    new MovementAction("JumpLeft_m3_m1", -3, -1, 1.5f, 0.4f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X - 3, p.Y - 3)),
//      //    new MovementAction("JumpLeft_m2_m1", -3, -1, 1.5f, 0.3f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X - 3, p.Y - 3)),
//      //    new MovementAction("JumpLeft_m1_m1", -3, -1, 1.5f, 0.2f,
//      //        (p, ai) => ai.HasGroundBelow(p.X, p.Y) && ai.ClearHeadspace(p.X, p.Y, 3),
//      //        (p, ai) => new Point(p.X - 3, p.Y - 3)),

//      //    // Grande chute verticale (si vide en dessous)
//      //    new MovementAction("Fall", 0, 5, 1.0f, 1f,
//      //        (p, ai) => !ai.HasGroundBelow(p.X, p.Y),
//      //        (p, ai) => ai.SimulateFall(p))
//      //};
//    }
